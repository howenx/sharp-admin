...
//    var rateSet = $("#rateSet").val();//税率设置
    ...
    //验证输入数据合法性
    if (!numberReg2.test(itemPrice) || !numberReg2.test(itemSrcPrice) || !numberReg1.test(restrictAmount) || !numberReg1.test(restAmount) || invCode=="") {
        orSave = false;
//        alert("输入数据不合法!");
    }
    //上下架时间验证
...
//    var ss = moment(now).format("YYYYMMDD");
    ...
    //状态和时间的检查
    if ((state=="Y" && (startAt>nowTime || nowTime>endAt)) || (state=="P" && (nowTime>startAt || nowTime>endAt)) || (state=="D" && (startAt>nowTime || endAt>nowTime))) {
        orSave = false;
...
//            alert("限制销售量之和不能大于剩余库存");
    ...
    for(var item in trdobj){
        if ((count==0 && itemId>0) || count==1 || count==2 || count==3 || count==7 || count==9)
    ...
    if($(option).hasClass("add-guige")){
        ...
        $(".thval").each(function(){
            ...
            if ((thName=="SKU ID" && itemId>0) || thName=="规格" || thName=="规格编号" || thName=="状态" || thName=="现价" || thName=="库存量")
