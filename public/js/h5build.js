...
//        }
        ...
        if($("#onShelvesAt").val()=="" || $("#offShelvesAt").val()=="" || $("#h5-link").val()==""){
                    isPost = false;
                    $('#js-userinfo-error').text('基本信息不能为空!').css('color', '#c00');
                    setTimeout("$('#js-userinfo-error').text('').css('color', '#2fa900')",3000);
                    return false;
        ...
        if($("#offShelvesAt").val() > validDate || $("#onShelvesAt").val() > 0){
         ...
         var themeState = parseInt($("input[name='themestate']:checked").attr("id"));
          ...
          //分割后的图片
          var themeMasterImg = [];
          var img = $("#draga").find("input").val().split(",");
          themeMasterImg.push(img[0]);
          themeMasterImg.push(img[1]);
          themeMasterImg.push(img[2]);

          //标签功能
          var tags = [];
          $(".ui-draggable").each(function(){
                var object = new Object();
                var tag = $(this).attr("style").split(";");
                var top = tag[3].split(":")[1];
                var left = tag[4].split(":")[1];
                var width = tag[0].split(":")[1];
                var height = tag[1].split(":")[1];
          ...
          if(isPost){
             $.ajax({
                 type :  "POST",
                 url : "/topic/h5Builder/h5BuilderSave",
