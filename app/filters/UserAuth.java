...
//        Logger.info("url:"+header.get());
//        Logger.info("method:"+header2.get());
        if (header.isPresent()) {
            ...
            if(username != null) {
                ...
                if(user != null) {
                    ...
                    //过滤用户权限
                    String[] userTypeArr = user.userType().get().split(",");
                    Boolean flag = false;
                    for (String userType : userTypeArr) {
                        if (configuration.getStringList(userType).contains(header2.get())) {
                            flag = true;
                        }
                    }
                    if (flag) ctx.args.put("user",user);
                    else return null;
                }
                else {
                    return null;
